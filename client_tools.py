...
async def connect_to_MCP_server(address: str="http://127.0.0.1:8000/mcp") -> Client:
    return Client(address, timeout=10.0)


async def is_connected(client: Client) -> bool:
    async with client:
        return client.is_connected()


async def get_tool_list(client: Client) -> list:
    tools = []
    try:
        async with client:
...
async def lowlevel_call_MCP_server(MCP_client: Client, function_name: str, function_arguments: dict={}) -> list|str:
    result = []
    try:
        async with MCP_client:
            tool_result = await MCP_client.call_tool(function_name, function_arguments)
...
async def call_MCP_server(MCP_client: Client, response: dict) -> list|str:
    result = []
